...
```{r BVN, fig.height = 3, fig.width = 6}
...
```{r rectangular, fig.height = 3, fig.width = 6}
par(mfrow = c(1,2), pty = 's', mar = c(3,3,3,3), cex = 1, xpd = TRUE)
...
```{r irregular, fig.height = 3, fig.width = 6}
par(mfrow = c(1,2), pty = 's', mar = c(3,3,3,3), cex = 1, xpd = TRUE)
...
out$eIWS <- runsim(nrepl, traps = grid10, template = build.irregular, N = 20, detectfn = 'HHN',
                ...
                allinside = FALSE, gridaligned = TRUE, ncores = 7)
```
```{r saveout, echo = FALSE, eval = FALSE}
save(out, file= paste(vignettefolder, 'out.RData', sep=''))
```
...
```{r runsimrevised, echo = TRUE, eval = FALSE}
outr <- vector('list', 5)
...
outr$e <- runsim(nrepl, traps = grid10, template = build.irregular, N = 20, detectfn = 'HHN',
                ...
                allinside = FALSE, gridaligned = TRUE, ncores = 7)
```
```{r saveoutrevised, echo = FALSE, eval=FALSE}
